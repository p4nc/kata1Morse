...
class Translator(ttk.Frame):
    def __init__(self,parent,**kwargs): #kwargs es por convenio
        ...
        self.send_entry = ttk.Entry(self, width=16, textvariable=self.sender)
        self.send_entry.place(x=70, y=10)

        recerver_lbl = ttk.Label(self,text="Receiver:", width=11, anchor=W)
        recerver_lbl.place(x=250, y=12)
        ...
        btn_send.place(y=165, x=516)


        btn_change = ttk.Button(self, command=self.changeText, text="<=>")
        btn_change.place(y= 140, x= 516)

 


    def traduce(self, e): #la e en este caso no tiene ningun valor, la ponemos para hablar con .bind(linea 31)
    ...
    def changeText(self):
        ...
        if self.traduccionDirecta:
            ...
            self.origin_lbl.config(text="Pano")
